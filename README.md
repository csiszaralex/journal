...
- **Push notifications** — per-device timezone (12 presets), reminder hour and quarter-hour minute (default 21:00). The worker process (`* * * * *`) checks every enabled subscription each minute, and if the device's local clock matches `notify_hour:notify_minute` exactly and no daily entry exists for the local day, sends a deterministic prompt picked from [data/prompts.json](data/prompts.json) (45 prompts per language, index-aligned so a given day asks the same question in either). The worker has no request to negotiate a language from, so it uses the setting, falling back to the language the device asked for when it subscribed. Duplicate sends are blocked by `notifications_sent(subscription_id, date)`. `404`/`410` responses disable the subscription. Clicking the notification (or saving that day's entry) sends a `close` push so it is dismissed on every device. Test and "preview today's prompt" buttons live on `/settings/devices`.
...
- **Language** — English and Hungarian, switched from a card in Settings. Until it is switched once there is no stored choice and the language is negotiated from `Accept-Language`, so a fresh install speaks whatever the browser asks for. Dates follow it too: the format patterns are per-language, not one pattern with translated month names. The AI writes its questions and emotion suggestions in the same language, though **text already generated stays as it was** — the questions on past entries and the emotion names in the database are content, not interface, and are never rewritten.
